...
resource "aws_internet_gateway" "igw" {
   ...
   tags = {
        Name         = "igw-pub-sub-${var.stack}-${var.environment}"
        environment  = "${var.environment}"
   }
}
...
resource "aws_route_table" "pub_rtb" {
   ...
   tags = {
     Name        = "Public-RTB-${var.stack}-${var.environment}"
     environment  = "${var.environment}"
   }
}
...
resource "aws_subnet" "public-subnet" {
   ...
   vpc_id            = "${aws_vpc.vpc.id}"
   tags = {
        Name        = "Public_Subnet-${var.stack}-${var.environment}-${count.index}"
        environment  = "${var.environment}"
   }
}
...
resource "aws_subnet" "private-subnet" {
   ...
   vpc_id            = "${aws_vpc.vpc.id}"
   tags = {
        Name        = "Private_Subnet-${var.stack}-${var.environment}-${count.index}"
        environment  = "${var.environment}"
   }
...
resource "aws_route_table" "pri_rtb" {
   ...
   tags = {
     Name        = "Private-RTB-${var.stack}-${var.environment}"
     environment  = "${var.environment}"
   }
}
